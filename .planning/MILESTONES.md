...
- Pure-transform adapter `fromProvaModelo3()` extracts typed `AssessmentInput` from raw backend shape; `LayoutAvaliacao` updated to consume `AssessmentInput` with `QuestionInput` field names throughout.
