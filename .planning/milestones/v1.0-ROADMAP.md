...
This milestone refactors the library's public API surface: first establishing a clean, typed input contract and enforcing strict types on domain entities, then wiring up the `fromProvaModelo()` adapter and fixing known builder defects, and finally documenting the new contract so consuming developers can integrate without inspecting source code.
...
- [x] **Phase 2: Adapter & Builder** - Implement `fromProvaModelo()` adapter and clean up the builder API (completed 2026-06-01)
...
**Goal**: Consuming developers can pass a raw `provaModelo3` backend response through `fromProvaModelo()` and receive a valid `AssessmentInput`; the builder API exposes no dead or duplicated methods
**Mode:** mvp
**Depends on**: Phase 1
**Requirements**: INPUT-03, BUILDER-01, BUILDER-02
**Success Criteria** (what must be TRUE):

  1. `fromProvaModelo()` is exported as a named function from the package and accepts the current `provaModelo3` backend shape without any changes to the backend
  2. Calling `fromProvaModelo(provaModelo3)` in the browser dev harness produces an `AssessmentInput` that renders identically to the previous raw-input path (pixel-identical HTML output)
...
**Success Criteria** (what must be TRUE):
  ...
  2. README contains a `fromProvaModelo()` usage section explaining how existing consumers can migrate from the raw backend shape
...
- [x] 03-01-PLAN.md — Create README.md with AssessmentInput contract, builder reference, and fromProvaModelo migration guide (DOCS-01, DOCS-02)
