...
overrides_applied: 0
gap_closure_note: "Gaps from initial verification (2026-06-01) were resolved by plan 02-03 (commits c9a8b99 and 6898702): adapter listaProvaAnexo path corrected, quebraQuestao mapped, index.html wired to fromProvaModelo, marcaDaquaRascunho typo fixed."
gaps:
  - truth: "Calling fromProvaModelo(provaModelo3) in the browser dev harness produces an AssessmentInput that renders identically to the previous raw-input path (pixel-identical HTML output)"
    status: failed
    reason: "The browser dev harness (index.html:262) still passes raw provaModelo3 directly to build() instead of going through fromProvaModelo(). The harness was never updated. Additionally, the adapter reads listaProvaAnexo from the wrong level (root of provaModelo3) while the fixture places it at prova.listaProvaAnexo — attachments are always silently empty. The quebraQuestao field is at prova.quebraQuestao in the fixture but the adapter maps prova.layout to layout, so input.layout?.quebraQuestao is always undefined."
    artifacts:
      - path: "index.html"
    ...
    missing:
      - "Update index.html line 262 to: const assessmentInput = AvaliacaoLayout.fromProvaModelo(provaModelo3); let layoutResult = layoutBuilder.build(assessmentInput);"
...
human_verification:
  - test: "Run npm run dev and open http://localhost:10001 in a browser"
    expected: "After the harness is updated to call fromProvaModelo(provaModelo3) and then build(assessmentInput), the assessment renders with all questions visible, pagination works, and output is pixel-identical to Phase 1 output"
...
**Phase Goal:** Consuming developers can pass a raw `provaModelo3` backend response through `fromProvaModelo()` and receive a valid `AssessmentInput`; the builder API exposes no dead or duplicated methods
...
|---|-------|--------|----------|
| 1 | `fromProvaModelo()` exported as named function from the package | VERIFIED | `src/index.ts:11` — `export { fromProvaModelo } from './adapter/ProvaModelo3Adapter'`; function signature `export function fromProvaModelo(provaModelo3): AssessmentInput` confirmed in `src/adapter/ProvaModelo3Adapter.ts:2` |
| 2 | `fromProvaModelo()` callable from browser harness and returns AssessmentInput-shaped object with correct fields | PARTIAL — FAILED | Function exists with correct structural shape (id, title, questions, attachments, layout). However: (a) harness never calls it — still uses raw object; (b) `listaProvaAnexo` always resolves to `undefined` because the fixture places it at `prova.listaProvaAnexo`, not root level, so `attachments` is always `[]` |
...
|----------|----------|--------|---------|
| `src/adapter/ProvaModelo3Adapter.ts` | fromProvaModelo adapter function | STUB (partial) | Exists, exports `fromProvaModelo`, correct imports from `../types/AssessmentInput`, no JSON.parse. Defect: reads `listaProvaAnexo` from root (undefined in fixture); does not map `quebraQuestao` from `prova.quebraQuestao` into layout |
...
| `src/index.ts` | Public package entry with fromProvaModelo re-export | VERIFIED | Line 11: `export { fromProvaModelo } from './adapter/ProvaModelo3Adapter'` |
...
| `src/index.ts` | `src/adapter/ProvaModelo3Adapter.ts` | re-export | VERIFIED | Line 11: `export { fromProvaModelo } from './adapter/ProvaModelo3Adapter'` — direct form per D-04 |
...
| `index.html` | `src/adapter/ProvaModelo3Adapter.ts` | `fromProvaModelo()` call | NOT WIRED | Harness still calls `layoutBuilder.build(provaModelo3)` raw (line 262); never calls `fromProvaModelo()`. Also calls deleted `marcaDaquaRascunho` method (line 251) |
...
| fromProvaModelo in index.ts | `grep "fromProvaModelo" src/index.ts` | line 11 confirmed | PASS |
...
**Test:** After updating `index.html` to call `fromProvaModelo(provaModelo3)` and pass the result to `build()`, run `npm run dev` and open http://localhost:10001
...
**Gap 1: Browser harness not updated** — `index.html` still passes raw `provaModelo3` to `build()` (line 262) and still calls the deleted `marcaDaquaRascunho()` method (line 251). The end-to-end path `fromProvaModelo(raw) → build()` is never actually exercised in the dev harness. The mandatory human-verify gate from Plan 02-02 Task 2 was not executed.
